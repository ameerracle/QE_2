#!/usr/bin/env python3
"""
Generate and optionally run QE PDOS workflow with PAW pseudopotentials.
Output directory: /lustre10/scratch/anizami/QE_2/no_u/PDOS
"""
...
# Updated to the new PAW kjpaw_psl files
PSEUDOS = {
...
def write_scf_input(file_path: Path, atoms, tag: str, pseudo_dir: str, ecutwfc: float, ecutrho: float, degauss: float, k_scf: list[int]):
    nat = len(atoms)
...
def write_nscf_input(file_path, atoms, tag, pseudo_dir, ecutwfc, ecutrho, k_nscf):
    nat = len(atoms)
    species = ordered_species(tag, atoms)
    ntyp = len(species)
...
def write_pp_input(file_path, tag, filplot, fileout):
    with open(file_path, "w", encoding="utf-8") as f:
        f.write("&INPUTPP\n")
        f.write(f"  prefix = '{tag}'\n")
        f.write("  outdir = './tmp/'\n")
...
def run_chain(run_dir, tag, np, step, do_pdos):
    steps = []
...
def main():
    parser = argparse.ArgumentParser(description="Generate/run PAW PDOS workflow")
    parser.add_argument("--slab", choices=SLABS, help="Run only one slab family")
    parser.add_argument("--ads", nargs="+", default=["Li2S4", "S8", "Li2S8"])
    parser.add_argument("--xyz-dir", type=Path, default=Path("/lustre10/scratch/anizami/QE_2/no_u/final_combi"))
    ...
    tags = []
    for xyz in sorted(args.xyz_dir.glob("*_combi_final.xyz")):
        tag = xyz.name.removesuffix("_combi_final.xyz")
        slab, ads = tag.split("_", 1)
        if (args.slab and slab != args.slab) or (ads not in args.ads): continue
        tags.append(tag)

    if not tags: return print("No matching xyz files found.")

    for tag in tags:
        xyz_file = args.xyz_dir / f"{tag}_combi_final.xyz"
        atoms = read(str(xyz_file))
        run_dir = args.run_root / tag
        run_dir.mkdir(parents=True, exist_ok=True)
        (run_dir / "tmp").mkdir(exist_ok=True)

        # Always generate SCF input (needed for structure, and for NSCF if SCF was done)
        write_scf_input(run_dir / f"{tag}_scf.in", atoms, tag, args.pseudo_dir, args.ecutwfc, args.ecutrho, args.degauss, k_scf)
        
        # Determine if PDOS is needed based on mode
        ads_name = tag.split("_")[1]
        if args.mode == "full":
            do_pdos = True
        ...
        # Generate PP input if we might need it (SCF step or full step)
        if args.step in ["scf", "full"]:
            write_pp_input(run_dir / f"{tag}_pp.in", tag, f"{tag}_rho", f"{tag}_charge.cube")
        
        # Generate NSCF/PROJWFC inputs if PDOS is needed and step requires it
        if do_pdos and args.step in ["nscf", "full"]:
            write_nscf_input(run_dir / f"{tag}_nscf.in", atoms, tag, args.pseudo_dir, args.ecutwfc, args.ecutrho, k_nscf)
            write_projwfc_input(run_dir / f"{tag}_projwfc.in", tag, args.filpdos, args.deltae, args.emin, args.emax)
        
        print(f"Generated inputs for {tag} (mode={args.mode}, step={args.step}, do_pdos={do_pdos}) in {run_dir}")
        if args.run:
