#!/usr/bin/env python3
"""
Generate QE PDOS workflow for adsorbate-only (Li/S) extracted from relaxed combi structures.
Outputs to /lustre10/scratch/anizami/QE_2/no_u/PDOS/adsorbates_combi/
"""
...
def write_scf_input(file_path, atoms, system_name, pseudo_dir, ecutwfc, ecutrho, degauss, k_scf):
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"  occupations = 'smearing', smearing = 'gaussian', degauss = {degauss:.5f}, nspin = 1\n/\n")
...
def write_nscf_input(file_path, atoms, system_name, pseudo_dir, ecutwfc, ecutrho, k_nscf):
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"&SYSTEM\n  ibrav = 0, nat = {len(atoms)}, ntyp = {ntyp}\n")
        f.write(f"  ecutwfc = {ecutwfc:.1f}, ecutrho = {ecutrho:.1f}\n")
        f.write("  occupations = 'smearing', smearing = 'gaussian', degauss = 0.005, nspin = 1\n/\n")
...
def prepare_single_system(system_name: str, input_root: Path, run_root: Path, pseudo_dir: str, **kwargs):
    xyz_file = input_root / f"{system_name}_combi_final.xyz"
...
def main():
    ...
    parser.add_argument("--input-root", type=Path, default=Path("/lustre10/scratch/anizami/QE_2/no_u/combi_144/PDOS/final_xyz"))
    parser.add_argument("--run-root", type=Path, default=Path("/lustre10/scratch/anizami/QE_2/no_u/PDOS/adsorbates_combi"))
