"""
bader_analysis.py -- Bader charge analysis for TMN slab + adsorbate systems
Run from: /lustre10/scratch/anizami/QE_2/

Workflow:
  1. Runs `bader <cube>` in each relevant directory
  2. Parses ACF.dat for each system
  3. Reads atom positions from cube file header
  4. Filters surface layer atoms by z-cutoff
  5. Outputs one CSV per metal+adsorbate combo
...
SLAB_BASE   = PDOS_BASE
ADS_BASE    = BASE_DIR / "adsorbates/PDOS"
...
def analyze_system(metal_slab, adsorbate, force=False):
    ...
    run bader, parse charges, build comparison dataframe.
    """
    ...
    slab_dir   = SLAB_BASE  / f"{metal_slab}_slab"
    slab_cube  = find_cube_file(slab_dir, preferred_name=f"{metal_slab}_charge.cube")

    ads_dir    = ADS_BASE / adsorbate
    ads_cube   = find_cube_file(ads_dir, preferred_name=f"{adsorbate}_charge.cube")

    for label, path in [("combi", combi_cube),
                        ("slab",  slab_cube),
                        ("ads",   ads_cube)]:
        if path is None or not path.exists():
            print(f"  ERROR: {label} cube not found in expected location")
            return None

    # Run bader
    acf_combi = run_bader(combi_cube, acf_label=system_key, force=force)
    acf_slab  = run_bader(slab_cube,  acf_label=metal_slab, force=force)
    acf_ads   = run_bader(ads_cube,   acf_label=adsorbate, force=force)
    ...
    for j in range(n_li):
        ...
        rows.append({
            ...
            "q_combi":    q_combi[i_combi],
            "q_ref":      q_ads[i_ads],
            "delta_q":    q_combi[i_combi] - q_ads[i_ads],
        })
    ...
    print(df.groupby("region")[["q_combi", "q_ref", "delta_q"]].mean().round(4))
