...
def write_scf_input(file_path: Path, atoms, tag: str, degauss: float, k_scf: list[int]) -> None:
    nat = len(atoms)
...
def write_nscf_input(file_path: Path, atoms, tag: str, k_nscf: list[int]) -> None:
    nat = len(atoms)
    species = ordered_species(tag, atoms)
    ntyp = len(species)
...
def main() -> None:
    ...
    parser.add_argument("--k-nscf", default="8 8 1 0 0 0", help="Denser grid for NSCF LOBSTER step")
    ...
    for xyz in sorted(XYZ_DIR.glob("*_final.xyz")):
        ...
        write_scf_input(run_dir / f"{tag}_scf.in", atoms, tag, args.degauss, k_scf)
        write_nscf_input(run_dir / f"{tag}_nscf.in", atoms, tag, k_nscf)
